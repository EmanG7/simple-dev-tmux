...
WBG_WIDGET=$(tmux show-option -gv @simple-dev-tmux_show_wbg 2>/dev/null)
if [ "$WBG_WIDGET" == "0" ]; then
	          exit 0
fi
...
PROVIDER=$(git config remote.origin.url | sed 's|https://||' | sed 's|git@||' | awk -F'[:/]' '{print $1}'i)
